...
@section('backend-content-wrapper')
    <div class="container-fluid user-add-wrapper">
        <form id="product-add-form" method="post" action="{{ route('admin.customers.update', ['customer' => $customer]) }}">
            @csrf
            <div class="row">
                <div class="col-12">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                          <li class="breadcrumb-item"><a href="{{ route('admin.customers.index') }}">{{ __('app.customers.all-customers') }}</a></li>
                          <li class="breadcrumb-item active" aria-current="page">{{ __('app.customers.edit-title') }}</li>
                        </ol>
                    </nav>
                </div>
                ...
                <div class="col-12 d-inline-flex justify-content-between align-items-center mb-4" id="fix-header-content">
                    <div class="content-title">
                        <h2>{{ __('app.customers.edit-title') }}</h2>
                    </div>
